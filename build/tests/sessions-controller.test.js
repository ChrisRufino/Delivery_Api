...
// src/tests/sessions-controller.test.ts
...
function errorHandlingMiddleware(error, request2, response, next) {
...
var envSchema = import_zod3.z.object({
  ...
  JWT_SECRET: import_zod3.z.string().min(1),
  PORT: import_zod3.z.string().transform((value) => parseInt(value, 10))
});
...
var DeliveriesController = class {
  async create(request2, response) {
    const bodySchema = import_zod5.z.object({
      ...
      description: import_zod5.z.string().trim().min(3)
    });
    const { user_id, description } = bodySchema.parse(request2.body);
...
function ensureAuthenticated(request2, response, next) {
  try {
    ...
    const { sub: user_id, role } = (0, import_jsonwebtoken2.verify)(
      token,
...
var DeliveriesStatusController = class {
  async update(request2, response) {
    ...
    const bodySchema = import_zod6.z.object({
      status: import_zod6.z.enum(["processing", "shipped", "delivered"])
    });
    const { id } = paramSchema.parse(request2.params);
...
var DeliveryLogsController = class {
  async create(request2, response) {
    ...
    const { delivery_id, description } = bodySchema.parse(request2.body);
  ...
  async show(request2, response) {
    const paramsSchema = import_zod7.z.object({
      delivery_id: import_zod7.z.string().uuid()
    });
    const { delivery_id } = paramsSchema.parse(request2.params);
    const delivery = await prisma.delivery.findUnique({
    ...
    if (request2.user?.role === "customer" && request2.user.id !== delivery?.userId) {
...
describe("sessionsController", () => {
  ...
  it("should authenticate a and get acess token", async () => {
    const userResponse = await (0, import_supertest.default)(app).post("/users").send({
      name: "Test User",
      email: "tesstuser@example.com",
      password: "password123"
    });
    user_id = userResponse.body.id;
    const sessionResponse = await (0, import_supertest.default)(app).post("/sessions").send({
      email: "tesstuser@example.com",
      password: "password123"
    });
